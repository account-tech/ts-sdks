...
export function isField(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V31}::dynamic_field::Field` + '<'); }
...
export class Field<Name extends TypeArgument, Value extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static fromSuiObjectData<Name extends Reified<TypeArgument, any>, Value extends Reified<TypeArgument, any>>( typeArgs: [Name, Value], data: SuiObjectData ): Field<ToTypeArgument<Name>, ToTypeArgument<Value>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isField(data.bcs.type)) { throw new Error(`object at is not a Field object`); }
