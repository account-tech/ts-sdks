...
export function isActionRequest(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V28}::token::ActionRequest` + '<'); }
...
export class ActionRequest<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V28}::token::ActionRequest`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = ActionRequest.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::token::ActionRequest<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = ActionRequest.$isPhantom;
 ...
 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: ActionRequestFields<T>, ) { this.$fullTypeName = composeSuiType( ActionRequest.$typeName, ...typeArgs ) as `${typeof PKG_V28}::token::ActionRequest<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;
...
export function isRuleKey(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V28}::token::RuleKey` + '<'); }
...
export function isToken(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V28}::token::Token` + '<'); }
...
export class Token<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V28}::token::Token`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = Token.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::token::Token<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = Token.$isPhantom;

 readonly id: ToField<UID>; readonly balance: ToField<Balance<T>>

 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: TokenFields<T>, ) { this.$fullTypeName = composeSuiType( Token.$typeName, ...typeArgs ) as `${typeof PKG_V28}::token::Token<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.balance = fields.balance; }

 static reified<T extends PhantomReified<PhantomTypeArgument>>( T: T ): TokenReified<ToPhantomTypeArgument<T>> { return { typeName: Token.$typeName, fullTypeName: composeSuiType( Token.$typeName, ...[extractType(T)] ) as `${typeof PKG_V28}::token::Token<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>], isPhantom: Token.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => Token.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Token.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => Token.fromBcs( T, data, ), bcs: Token.bcs, fromJSONField: (field: any) => Token.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => Token.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => Token.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => Token.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => Token.fetch( client, T, id, ), new: ( fields: TokenFields<ToPhantomTypeArgument<T>>, ) => { return new Token( [extractType(T)], fields ) }, kind: "StructClassReified", } }
...
export function isTokenPolicy(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V28}::token::TokenPolicy` + '<'); }
...
export function isTokenPolicyCap(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V28}::token::TokenPolicyCap` + '<'); }
...
export function isTokenPolicyCreated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V28}::token::TokenPolicyCreated` + '<'); }
