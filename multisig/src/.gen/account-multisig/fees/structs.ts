...
export interface FeesFields { id: ToField<UID>; amount: ToField<"u64">; recipient: ToField<"address"> }
...
export class Fees implements StructClass { __StructClass = true as const;
 ...
 readonly id: ToField<UID>; readonly amount: ToField<"u64">; readonly recipient: ToField<"address">
 ...
 this.id = fields.id;; this.amount = fields.amount;; this.recipient = fields.recipient; }
 ...
 id: UID.bcs, amount: bcs.u64(), recipient: bcs.bytes(32).transform({ input: (val: string) => fromHEX(val), output: (val: Uint8Array) => toHEX(val), })

}) };

 static fromFields( fields: Record<string, any> ): Fees { return Fees.reified( ).new( { id: decodeFromFields(UID.reified(), fields.id), amount: decodeFromFields("u64", fields.amount), recipient: decodeFromFields("address", fields.recipient) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): Fees { if (!isFees(item.type)) { throw new Error("not a Fees type");

 }

 return Fees.reified( ).new( { id: decodeFromFieldsWithTypes(UID.reified(), item.fields.id), amount: decodeFromFieldsWithTypes("u64", item.fields.amount), recipient: decodeFromFieldsWithTypes("address", item.fields.recipient) } ) }

 static fromBcs( data: Uint8Array ): Fees { return Fees.fromFields( Fees.bcs.parse(data) ) }

 toJSONField() { return {

 id: this.id,amount: this.amount.toString(),recipient: this.recipient,

} }

 toJSON() { return { $typeName: this.$typeName, $typeArgs: this.$typeArgs, ...this.toJSONField() } }

 static fromJSONField( field: any ): Fees { return Fees.reified( ).new( { id: decodeFromJSONField(UID.reified(), field.id), amount: decodeFromJSONField("u64", field.amount), recipient: decodeFromJSONField("address", field.recipient) } ) }
