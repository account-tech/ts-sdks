...
export function isRule(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::kiosk_lock_rule::Rule`; }
...
export class Rule implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [], fields: RuleFields, ) { this.$fullTypeName = composeSuiType( Rule.$typeName, ...typeArgs ) as `${typeof PKG_V1}::kiosk_lock_rule::Rule`; this.$typeArgs = typeArgs;
...
export function isConfig(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::kiosk_lock_rule::Config`; }
...
export class Config implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [], fields: ConfigFields, ) { this.$fullTypeName = composeSuiType( Config.$typeName, ...typeArgs ) as `${typeof PKG_V1}::kiosk_lock_rule::Config`; this.$typeArgs = typeArgs;
