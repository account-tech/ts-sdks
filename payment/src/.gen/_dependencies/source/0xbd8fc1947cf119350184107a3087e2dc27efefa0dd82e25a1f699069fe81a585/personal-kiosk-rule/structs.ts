...
export function isRule(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::personal_kiosk_rule::Rule`; }
...
export class Rule implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V1}::personal_kiosk_rule::Rule`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Rule.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::personal_kiosk_rule::Rule`; readonly $typeArgs: []; readonly $isPhantom = Rule.$isPhantom;

 readonly dummyField: ToField<"bool">

 private constructor(typeArgs: [], fields: RuleFields, ) { this.$fullTypeName = composeSuiType( Rule.$typeName, ...typeArgs ) as `${typeof PKG_V1}::personal_kiosk_rule::Rule`; this.$typeArgs = typeArgs;

 this.dummyField = fields.dummyField; }

 static reified( ): RuleReified { return { typeName: Rule.$typeName, fullTypeName: composeSuiType( Rule.$typeName, ...[] ) as `${typeof PKG_V1}::personal_kiosk_rule::Rule`, typeArgs: [ ] as [], isPhantom: Rule.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Rule.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Rule.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Rule.fromBcs( data, ), bcs: Rule.bcs, fromJSONField: (field: any) => Rule.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Rule.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Rule.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Rule.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Rule.fetch( client, id, ), new: ( fields: RuleFields, ) => { return new Rule( [], fields ) }, kind: "StructClassReified", } }
