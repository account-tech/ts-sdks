...
export function isLinkedTable(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V31}::linked_table::LinkedTable` + '<'); }
...
export class LinkedTable<K extends TypeArgument, V extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 id: this.id,size: this.size.toString(),head: fieldToJSON<Option<K>>(`${Option.$typeName}<${this.$typeArgs[0]}>`, this.head),tail: fieldToJSON<Option<K>>(`${Option.$typeName}<${this.$typeArgs[0]}>`, this.tail),
 ...
 static fromSuiObjectData<K extends Reified<TypeArgument, any>, V extends PhantomReified<PhantomTypeArgument>>( typeArgs: [K, V], data: SuiObjectData ): LinkedTable<ToTypeArgument<K>, ToPhantomTypeArgument<V>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isLinkedTable(data.bcs.type)) { throw new Error(`object at is not a LinkedTable object`); }
...
export function isNode(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V31}::linked_table::Node` + '<'); }
...
export class Node<K extends TypeArgument, V extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 prev: fieldToJSON<Option<K>>(`${Option.$typeName}<${this.$typeArgs[0]}>`, this.prev),next: fieldToJSON<Option<K>>(`${Option.$typeName}<${this.$typeArgs[0]}>`, this.next),value: fieldToJSON<V>(this.$typeArgs[1], this.value),
 ...
 static fromSuiObjectData<K extends Reified<TypeArgument, any>, V extends Reified<TypeArgument, any>>( typeArgs: [K, V], data: SuiObjectData ): Node<ToTypeArgument<K>, ToTypeArgument<V>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isNode(data.bcs.type)) { throw new Error(`object at is not a Node object`); }
