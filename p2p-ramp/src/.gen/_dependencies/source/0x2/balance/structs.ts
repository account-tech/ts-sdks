...
export function isSupply(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V29}::balance::Supply` + '<'); }
...
export function isBalance(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V29}::balance::Balance` + '<'); }
