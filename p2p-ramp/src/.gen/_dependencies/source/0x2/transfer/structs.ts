...
export function isReceiving(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V31}::transfer::Receiving` + '<'); }
...
export class Receiving<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V31}::transfer::Receiving`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = Receiving.$typeName; readonly $fullTypeName: `${typeof PKG_V31}::transfer::Receiving<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = Receiving.$isPhantom;

 readonly id: ToField<ID>; readonly version: ToField<"u64">

 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: ReceivingFields<T>, ) { this.$fullTypeName = composeSuiType( Receiving.$typeName, ...typeArgs ) as `${typeof PKG_V31}::transfer::Receiving<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.version = fields.version; }

 static reified<T extends PhantomReified<PhantomTypeArgument>>( T: T ): ReceivingReified<ToPhantomTypeArgument<T>> { return { typeName: Receiving.$typeName, fullTypeName: composeSuiType( Receiving.$typeName, ...[extractType(T)] ) as `${typeof PKG_V31}::transfer::Receiving<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>], isPhantom: Receiving.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => Receiving.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Receiving.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => Receiving.fromBcs( T, data, ), bcs: Receiving.bcs, fromJSONField: (field: any) => Receiving.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => Receiving.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => Receiving.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => Receiving.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => Receiving.fetch( client, T, id, ), new: ( fields: ReceivingFields<ToPhantomTypeArgument<T>>, ) => { return new Receiving( [extractType(T)], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, data: SuiObjectData ): Receiving<ToPhantomTypeArgument<T>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isReceiving(data.bcs.type)) { throw new Error(`object at is not a Receiving object`); }
