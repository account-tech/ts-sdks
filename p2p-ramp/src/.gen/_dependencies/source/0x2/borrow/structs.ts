...
export function isReferent(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V31}::borrow::Referent` + '<'); }
...
export class Referent<T extends TypeArgument> implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V31}::borrow::Referent`; static readonly $numTypeParams = 1; static readonly $isPhantom = [false,] as const;

 readonly $typeName = Referent.$typeName; readonly $fullTypeName: `${typeof PKG_V31}::borrow::Referent<${ToTypeStr<T>}>`; readonly $typeArgs: [ToTypeStr<T>]; readonly $isPhantom = Referent.$isPhantom;

 readonly id: ToField<"address">; readonly value: ToField<Option<T>>

 private constructor(typeArgs: [ToTypeStr<T>], fields: ReferentFields<T>, ) { this.$fullTypeName = composeSuiType( Referent.$typeName, ...typeArgs ) as `${typeof PKG_V31}::borrow::Referent<${ToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.value = fields.value; }

 static reified<T extends Reified<TypeArgument, any>>( T: T ): ReferentReified<ToTypeArgument<T>> { return { typeName: Referent.$typeName, fullTypeName: composeSuiType( Referent.$typeName, ...[extractType(T)] ) as `${typeof PKG_V31}::borrow::Referent<${ToTypeStr<ToTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [ToTypeStr<ToTypeArgument<T>>], isPhantom: Referent.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => Referent.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Referent.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => Referent.fromBcs( T, data, ), bcs: Referent.bcs(toBcs(T)), fromJSONField: (field: any) => Referent.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => Referent.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => Referent.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => Referent.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => Referent.fetch( client, T, id, ), new: ( fields: ReferentFields<ToTypeArgument<T>>, ) => { return new Referent( [extractType(T)], fields ) }, kind: "StructClassReified", } }
 ...
 id: this.id,value: fieldToJSON<Option<T>>(`${Option.$typeName}<${this.$typeArgs[0]}>`, this.value),
 ...
 static fromSuiObjectData<T extends Reified<TypeArgument, any>>( typeArg: T, data: SuiObjectData ): Referent<ToTypeArgument<T>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isReferent(data.bcs.type)) { throw new Error(`object at is not a Referent object`); }
...
export function isBorrow(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V31}::borrow::Borrow`; }
